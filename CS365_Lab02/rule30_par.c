...
int main(int argc, char **argv)
{
	...
	local->cur = malloc(sizeof(uint8_t *) * chunk_size);
	local->next = malloc(sizeof(uint8_t *) * chunk_size);
	...
	for (int i = 1; i <= num_gens; i++) {
		...
		uint8_t left_bound = 0;
		uint8_t right_bound = 0;

		// send left?
		if (rank != 0) {
            //printf("sending left from process %d\n", rank);
			MPI_Send(&local->cur[0], 1, MPI_CHAR, rank - 1, 0, MPI_COMM_WORLD);
		}

		// receive from right?
		if (rank != (size - 1)) {
            //printf("receiving right from process %d\n", rank);
			MPI_Recv(&right_bound, 1, MPI_CHAR, rank + 1, MPI_ANY_TAG,
		...
		if (rank != (size - 1)) {
			...
			MPI_Send(&local->cur[last], 1, MPI_CHAR, rank + 1, 0, MPI_COMM_WORLD);
		}

		// receive from left?
		if (rank != 0) {
            //printf("receiving left from process %d\n", rank);
			MPI_Recv(&left_bound, 1, MPI_CHAR, rank - 1, MPI_ANY_TAG,
					 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
	...
	if (rank == 0) {
        ...
        MPI_Send(&local->cur[0], chunk_size, MPI_CHAR, 0, 0, MPI_COMM_WORLD);
	}
